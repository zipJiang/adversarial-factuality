...
for filename in sampled_dev.txt sampled_train.txt sampled_test.txt; do
    ...
    export OUTPUT_PATH=/brtx/606-nvme2/zpjiang/adversarial-factuality/data/generated-samples/${filename%.txt}-info.jsonl
    conda run -p $ENV_PATH --no-capture-output \
        python3 scripts/run_task.py configs/generation/sample_info_violation.yaml
